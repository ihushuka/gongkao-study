...
export default function App() {
  ...
  const beginTimerWithTask = (title: string, module: string, taskKey = "", source = "") => {
    ...
    setTimerStartedAt(new Date()); setTimerOn(true);
  };
  const toggleTimer = () => {
    if (!timerOn) {
      if (!timerTitle.trim()) { flash("请先选择当日任务或填写具体学习内容"); return; }
      setTimerStartedAt(new Date()); setTimerOn(true); return;
    }
    ...
    setTimerOn(false); setTimerStartedAt(null); setActiveSeconds(0); flash("本次学习时长已保存，并同步到实际学习时间轴");
  };
  ...
  const sharedTimer = { timerOn, activeSeconds, timerModule, setTimerModule, timerTitle, setTimerTitle, timerSource, setTimerSource, timerTaskKey, setTimerTaskKey, beginTimerWithTask, toggleTimer, todaySeconds, timerStartedAt };
  let page;
...
type TimerShared = { timerOn: boolean; activeSeconds: number; timerModule: string; setTimerModule: Dispatch<SetStateAction<string>>; timerTitle: string; setTimerTitle: Dispatch<SetStateAction<string>>; timerSource: string; setTimerSource: Dispatch<SetStateAction<string>>; timerTaskKey: string; setTimerTaskKey: Dispatch<SetStateAction<string>>; beginTimerWithTask: (title: string, module: string, taskKey?: string, source?: string) => void; toggleTimer: () => void; todaySeconds: number; timerStartedAt: Date | null };
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; timer: TimerShared }) {
  ...
  const actual = sessions.filter(s => normalizedDate(s.date) === date).map(s => ({ ...s, title: sessionDisplayTitle(s, tasks, routines), module: sessionDisplayModule(s, tasks, routines), running: false }));
  if (date === localISO() && timer.timerOn && timer.timerStartedAt) actual.push({ id: -1, date, module: timer.timerModule, title: timer.timerTitle, source: timer.timerSource, linkedTaskKey: timer.timerTaskKey || undefined, seconds: timer.activeSeconds, startTime: `${pad(timer.timerStartedAt.getHours())}:${pad(timer.timerStartedAt.getMinutes())}`, endTime: `${pad(new Date().getHours())}:${pad(new Date().getMinutes())}`, running: true });
  ...
  return <div className="page-stack"><section className="page-intro daily-intro"><div><p className="eyebrow">DAILY TIMELINE</p><h2>计划时间与真实投入并排看</h2><p>左边是预计安排，右边由学习计时自动生成实际完成时段。</p><span className="sync-badge">✓ 已与月度计划实时同步</span></div><input type="date" value={date} onChange={e => setDate(e.target.value)} /></section>
    ...
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button className="timeline-reset" onClick={() => setTimelineZoom(100)}>↺ 恢复标准</button></div><div className="timeline-head"><span>预计任务安排</span><b>时间</b><span>实际学习记录</span></div><div className="daily-timeline" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{plannedLayout.map(({ item, lane, laneCount }) => <article title={`${item.title} · ${item.start}–${item.end} · 点击修改预计时间`} key={`${item.kind}${item.id}`} className={`${item.done ? "done" : ""} ${item.state?.canceled ? "canceled" : ""} ${(item.state?.delayCount || 0) >= 2 ? "repeated-delay" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}><button className="timeline-check" onClick={event => { event.stopPropagation(); togglePlanned(item); }}>{item.done ? "✓" : ""}</button><button className="timeline-edit-trigger" title="修改预计时间" aria-label={`修改${item.title}的预计时间`} onClick={event => { event.stopPropagation(); beginEdit(item); }}>✎</button><div className="timeline-inline" onClick={() => beginEdit(item)}><span className="timeline-time">{item.start}–{item.end}</span><strong>{item.title}</strong><small>{item.subject}{item.state?.canceled ? " · 今日取消" : item.state?.delayCount ? ` · 已推迟${item.state.delayCount}次` : ""}</small></div></article>)}</div><div className="timeline-axis">{Array.from({ length: 19 }, (_, i) => i + 6).map((h, index) => <div key={h} style={{ top: `${index * 100 / 18}%` }}><span>{pad(h)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actualLayout.map(({ item, lane, laneCount }) => <article title={`${item.title || item.module} · ${item.startTime}–${item.endTime || "进行中"}`} key={item.id} className={`${item.running ? "running" : ""} ${timelineDensity(item.startTime, item.endTime)}`} style={{ top: `${timelineTop(item.startTime)}%`, height: `${timelineHeight(item.startTime, item.endTime)}%`, ...laneStyle(lane, laneCount) }}><div className="timeline-inline"><span className="timeline-time">{item.startTime}–{item.endTime || "进行中"}</span><strong>{item.title || item.module}</strong><small>{item.module} · {formatHours(item.seconds)}</small></div></article>)}</div></div></section>
    {editing && <div className="timeline-edit-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setEditing(null); }}><div className="timeline-edit-dialog" role="dialog" aria-modal="true" aria-label="修改预计学习时间"><div className="timeline-edit-dialog-head"><div><p className="eyebrow">PLANNED TIME</p><h3>修改预计学习时间</h3></div><button type="button" aria-label="关闭" onClick={() => setEditing(null)}>×</button></div><label>学习内容<input value={editTitle} onChange={e => setEditTitle(e.target.value)} /></label><div className="timeline-edit-time-grid"><label>预计开始<input type="time" value={editStart} onChange={e => setEditStart(e.target.value)} /></label><label>预计结束<input type="time" value={editEnd} onChange={e => setEditEnd(e.target.value)} /></label></div><p className="timeline-edit-duration">预计时长：{validTimeRange(editStart, editEnd) ? `${(timeMinutes(editEnd) || 0) - (timeMinutes(editStart) || 0)} 分钟` : "结束时间必须晚于开始时间"}</p><div className="timeline-edit-actions"><button className="soft-button" type="button" onClick={() => setEditing(null)}>取消</button><button className="primary-button" type="button" onClick={saveEdit}>保存并同步月度计划</button></div></div></div>}
  </div>;
}
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared }) {
  ...
  const taskChoicesFor = (targetDate: string) => [
    ...routines.filter(r => r.month === targetDate.slice(0, 7) && routineApplies(r, targetDate)).map(r => ({ key: makeTaskKey("routine", r.id), title: r.title, subject: r.subject, start: r.plannedStart, end: r.plannedEnd, kind: "每日任务" })),
    ...tasks.filter(t => normalizedDate(t.date) === targetDate).map(t => ({ key: makeTaskKey("task", t.id), title: t.title, subject: t.subject, start: t.plannedStart, end: t.plannedEnd, kind: "单日任务" })),
  ];

...
function PracticeView({ practices, setPractices, flash, moduleOptions, setModuleOptions, timer, taskPresets, setTaskPresets }: { practices: Practice[]; setPractices: Dispatch<SetStateAction<Practice[]>>; flash: (x: string) => void; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>>; timer: TimerShared; taskPresets: StudyTaskPreset[]; setTaskPresets: Dispatch<SetStateAction<StudyTaskPreset[]>> }) {
  ...
  const choosePracticeSource = (value: string) => {
    if (value.startsWith("task:")) {
      ...
      if (!preset) return;
      setLinkedPresetId(preset.id); setSource(preset.title); setModule(preset.subject); setLessonNumbers([]);
      return;
    }
    if (value.startsWith("source:")) { setLinkedPresetId(null); setSource(value.slice(7)); setLessonNumbers([]); return; }
    setLinkedPresetId(null); setSource(""); setLessonNumbers([]);
  };
  const togglePracticeLesson = (lesson: number) => setLessonNumbers(current => current.includes(lesson) ? current.filter(item => item !== lesson) : [...current, lesson].sort((a, b) => a - b));
  ...
  const add = (e: FormEvent) => { e.preventDefault(); if (!source.trim()) { flash("请选择练习来源或关联学习任务"); return; } if (correct > total) { flash("正确题数不能大于总题数，请检查后再保存"); return; } if (correct < 0 || total <= 0) { flash("题数必须是有效的正数"); return; } const linked = linkedPresetId !== null ? taskPresets.find(item => item.id === linkedPresetId) : undefined; const lessons = linked ? normalizedLessonNumbers(lessonNumbers, linked.totalLessons) : []; const row: Omit<Practice, "id"> = { date, source: linked?.title || source, module: linked?.subject || module, correct, total, minutes, presetId: linked?.id, lessonNumbers: lessons.length ? lessons : undefined }; if (editingId !== null) { setPractices(practices.map(p => p.id === editingId ? { ...p, ...row } : p)); setEditingId(null); flash(linked ? "练习记录已修改，并保持与学习任务及课时同步" : "练习记录已修改"); } else { setPractices([...practices, { id: Date.now(), ...row }]); flash(linked ? "练习成绩已记录，并已关联学习任务和对应课时" : "练习成绩已记录"); } setLessonNumbers([]); };
  const editPractice = (p: Practice) => { const linked = p.presetId !== undefined ? taskPresets.find(item => item.id === p.presetId) : undefined; setEditingId(p.id); setDate(normalizedDate(p.date)); setLinkedPresetId(linked?.id ?? null); setSource(linked?.title || p.source); setModule(linked?.subject || p.module); setLessonNumbers(linked ? normalizedLessonNumbers(p.lessonNumbers, linked.totalLessons) : []); setCorrect(p.correct); setTotal(p.total); setMinutes(p.minutes); };
  const cancelPracticeEdit = () => { setEditingId(null); setLinkedPresetId(null); setSource(""); setLessonNumbers([]); };

...
type Idiom = { id: number; word: string; meaning: string; example: string; level: "未掌握" | "模糊" | "已掌握"; source?: string; meaningHighlights?: string[]; ocrNote?: string; kind?: "成语" | "词语"; createdAt?: string; masteredAt?: string };
...
function Plan({ tasks, setTasks, routines, setRoutines, flash }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; flash: (x: string) => void }) {
  ...
  return <div className="page-stack">
    <section className="page-intro"><div><p className="eyebrow">MONTHLY PLAN</p><h2>把月目标拆到每一天</h2><p>与每日计划实时同步；点击月历日期可指定“一键清除一周”的所在周。</p></div><div className="intro-actions plan-actions"><input hidden ref={planFileRef} type="file" accept=".xlsx,.xls" onChange={importPlan} /><div className="clear-plan-group"><button className="clear-week-button" onClick={clearWeek}>清除所选周</button><button className="clear-month-button" onClick={clearMonth}>清除本月</button></div><button className="import-plan-button" onClick={() => planFileRef.current?.click()} disabled={importing}><span>表</span><div><strong>{importing ? "正在识别…" : "一键导入计划表"}</strong><small>自动识别日期与每日任务</small></div></button><input className="month-picker" type="month" value={month} onChange={e => { setMonth(e.target.value); if (!date.startsWith(e.target.value)) setDate(`${e.target.value}-01`); }} /></div></section>
...
function IdiomView({ idioms, setIdioms, flash }: { idioms: Idiom[]; setIdioms: Dispatch<SetStateAction<Idiom[]>>; flash: (x: string) => void }) {
  ...
  const confirmPreview = () => { const existing = new Set(idioms.map(x => `${x.source || ""}|${x.word}`)); const fresh = preview.filter(x => !existing.has(`${x.source || ""}|${x.word}`)).map((x, i) => ({ ...x, id: Date.now() + i, createdAt: new Date().toISOString() })); setIdioms([...idioms, ...fresh]); setPreview([]); flash(`已导入 ${fresh.length} 条人民日报成语`); };

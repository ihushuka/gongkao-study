...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer, moduleOptions, setModuleOptions }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>> }) {
  ...
  const [timeBandScope, setTimeBandScope] = useStoredState<"30" | "all">("shore-study-timeband-scope", "30");
  ...
  const timeBandStartDate = shiftISODate(localISO(), -29);
  const timeBandRows = timeBandScope === "all" ? sessions : sessions.filter(session => normalizedDate(session.date) >= timeBandStartDate && normalizedDate(session.date) <= localISO());
  const timeBandMinutes = Array.from({ length: 12 }, () => 0);
  const addTimeBandSegment = (segmentStart: number, segmentEnd: number) => {
    for (let band = 0; band < 12; band += 1) {
      const bandStart = band * 120, bandEnd = bandStart + 120;
      timeBandMinutes[band] += Math.max(0, Math.min(segmentEnd, bandEnd) - Math.max(segmentStart, bandStart));
    }
  };
  timeBandRows.forEach(session => {
    ...
    addTimeBandSegment(rawStart, Math.min(finalEnd, 1440));
    if (finalEnd > 1440) addTimeBandSegment(0, Math.min(1440, finalEnd - 1440));
  });
  const timeBands = timeBandMinutes.map((minutes, index) => ({ label: `${pad(index * 2)}–${pad(index * 2 + 2)}`, minutes }));
  const maxTimeBand = Math.max(...timeBands.map(item => item.minutes), 1);
  const totalTimeBandMinutes = timeBands.reduce((sum, item) => sum + item.minutes, 0);
  ...
  const studyModuleContent = (id: StudyModuleId) => {
    ...
    if (id === "timeband") return <div className="panel study-timeband-panel"><div className="panel-title study-panel-title-with-tabs"><div><h2>学习时间段分布</h2><p>按实际起止时间累计到2小时时段，帮助识别最常学习的时间。</p></div><div className="range-tabs"><button type="button" className={timeBandScope === "30" ? "active" : ""} onClick={() => setTimeBandScope("30")}>近30天</button><button type="button" className={timeBandScope === "all" ? "active" : ""} onClick={() => setTimeBandScope("all")}>总体</button></div></div><div className="timeband-summary"><strong>{Math.round(totalTimeBandMinutes / 6) / 10}h</strong><span>{timeBandScope === "30" ? "近30天有具体起止时间的记录" : "全部有具体起止时间的记录"}</span></div><div className="timeband-chart">{timeBands.map(item => <div className="timeband-column" key={item.label} title={`${item.label} · ${Math.round(item.minutes)}分钟`}><span>{item.minutes ? `${Math.round(item.minutes / 6) / 10}h` : ""}</span><div><i style={{ height: `${item.minutes ? Math.max(5, item.minutes / maxTimeBand * 118) : 2}px` }} /></div><small>{item.label}</small></div>)}</div><p className="timeband-note">没有填写开始/结束时间的历史记录不会计入该图；跨零点记录会自动拆分到对应时段。</p></div>;

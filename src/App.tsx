...
type StudySession = { id: number; date: string; module: string; seconds: number; startTime?: string; endTime?: string; title?: string; source?: string; linkedTaskKey?: string };
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; timer: TimerShared }) {
  ...
  const actualBlocks = [
    ...timelineSessionSources.flatMap(session => clockRangeSegmentsForDate(session.date, date, session.startTime, session.endTime).map((slot, index) => ({ entryType: "study" as const, key: `study-${session.id}-${date}-${index}`, title: session.title || session.module, label: `${session.module} · ${formatHours(session.seconds)}${session.paused ? " · 已暂停" : ""}`, start: slot.start, end: slot.end, running: session.running, paused: session.paused, session }))),
  ...
  return <div className="page-stack">
    ...
    <section className="panel life-event-panel"><div className="life-event-head"><div><p className="eyebrow">DAILY LIFE</p><h2>日常事件记录</h2><span>记录吃饭、睡觉、健身、通勤、家务等；可放到时间轴左侧“预计”或右侧“实际”，不会进入月度计划表格。</span></div><strong>{selectedLifeEvents.length} 条</strong></div><form className="life-event-form" onSubmit={saveLifeEvent}><label>日期<input type="date" value={lifeDate} onChange={e => setLifeDate(e.target.value)} /></label><label>类型<input list="life-event-category-options" value={lifeCategory} onChange={e => setLifeCategory(e.target.value)} placeholder="可自定义，如 阅读、购物" /><datalist id="life-event-category-options">{defaultLifeEventCategories.map(category => <option key={category} value={category} />)}</datalist><small>可直接输入自定义类型</small></label><label>显示位置<select value={lifeSide} onChange={e => setLifeSide(e.target.value as LifeEventSide)}><option value="planned">左侧 · 预计安排</option><option value="actual">右侧 · 实际发生</option></select></label><label className="life-title-field">事件名称<input value={lifeTitle} onChange={e => setLifeTitle(e.target.value)} placeholder={`例如：${lifeCategory === "睡觉" ? "夜间睡眠" : lifeCategory === "健身" ? "力量训练" : lifeCategory === "吃饭" ? "午饭" : "日常小事"}`} /></label><div className="life-time-row"><label>开始时间<input type="time" value={lifeStart} onChange={e => setLifeStart(e.target.value)} /></label><label>结束时间<input type="time" value={lifeEnd} onChange={e => setLifeEnd(e.target.value)} /><small>支持跨零点，如 23:30–07:00</small></label></div><label className="life-note-field">备注<input value={lifeNote} onChange={e => setLifeNote(e.target.value)} placeholder="可选，例如：公司食堂、跑步5km" /></label><div className="life-form-actions">{lifeEditingId && <button className="soft-button" type="button" onClick={resetLifeForm}>取消修改</button>}<button className="primary-button" type="submit">{lifeEditingId ? "保存修改" : "＋ 添加到时间轴"}</button></div></form>{selectedLifeEvents.length ? <div className="life-event-list">{selectedLifeEvents.slice().sort((a, b) => (timeMinutes(a.startTime) || 0) - (timeMinutes(b.startTime) || 0)).map(event => <article key={event.id}><span className="life-event-icon">{lifeEventIcon(event.category)}</span><div><strong>{event.title}</strong><small>{normalizedDate(event.date) !== date ? `${formatShortDate(event.date)}起 · ` : ""}{event.category} · {formatStudyRange(event.startTime, event.endTime)} · {event.side === "planned" ? "左侧预计" : "右侧实际"}{event.note ? ` · ${event.note}` : ""}</small></div><button type="button" onClick={() => editLifeEvent(event)}>修改</button><button className="life-delete" type="button" onClick={() => { if (window.confirm(`删除“${event.title}”这条日常事件吗？`)) { setLifeEvents(current => current.filter(item => item.id !== event.id)); if (lifeEditingId === event.id) resetLifeForm(); } }}>删除</button></article>)}</div> : <div className="all-scheduled">{date} 还没有日常事件记录。</div>}</section>
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button className="timeline-reset" onClick={() => setTimelineZoom(100)}>↺ 恢复标准</button></div><div className="timeline-head"><span>预计任务 / 日常安排</span><b>时间</b><span>实际学习 / 日常记录</span></div><div className="daily-timeline timeline-full-day" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{plannedLayout.map(({ item, lane, laneCount }) => { const study = item.entryType === "study" ? item.item : null; const life = item.entryType === "life" ? item.lifeEvent : null; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : " · 点击修改预计时间段"}`} key={item.key} className={`${life ? "life-event-block" : ""} ${study?.done ? "done" : ""} ${study?.state?.canceled ? "canceled" : ""} ${(study?.state?.delayCount || 0) >= 2 ? "repeated-delay" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{study ? <button className="timeline-check" onClick={event => { event.stopPropagation(); togglePlanned(study); }}>{study.done ? "✓" : ""}</button> : <span className="timeline-life-dot">{life ? lifeEventIcon(life.category) : "记"}</span>}<button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改预计时间段"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (study) beginEdit(study); }}>✎</button><div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); else if (study) beginEdit(study); }}><span className="timeline-time">{item.start}–{item.end}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : study?.state?.canceled ? " · 今日取消" : study?.state?.delayCount ? ` · 已推迟${study.state.delayCount}次` : ""}</small></div></article>; })}</div><div className="timeline-axis">{Array.from({ length: 25 }, (_, i) => i).map((hour, index) => <div key={hour} style={{ top: `${index * 100 / 24}%` }}><span>{pad(hour)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actualLayout.map(({ item, lane, laneCount }) => { const life = item.entryType === "life" ? item.lifeEvent : null; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : ""}`} key={item.key} className={`${life ? "life-event-block" : ""} ${item.running ? "running" : ""} ${item.paused ? "paused" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{life && <><span className="timeline-life-dot">{lifeEventIcon(life.category)}</span><button className="timeline-edit-trigger" title="修改日常事件" aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); editLifeEvent(life); }}>✎</button></>}<div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); }}><span className="timeline-time">{item.start}–{item.end}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : ""}</small></div></article>; })}</div></div></section>
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer, moduleOptions }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared; moduleOptions: string[] }) {
  ...
  const [recordTitle, setRecordTitle] = useState(""), [recordSource, setRecordSource] = useState(""), [recordTaskKey, setRecordTaskKey] = useState("");
  ...
  const addManual = (e: FormEvent) => {
    ...
    const row: Omit<StudySession, "id"> = { date: normalizedDate(date), module: module.trim() || "其他计划", title: recordTitle.trim(), source: recordSource.trim() || undefined, linkedTaskKey: recordTaskKey || undefined, seconds: finalMinutes * 60, startTime, endTime };
    if (editingId !== null) {

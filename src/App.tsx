...
type LifeEvent = { id: number; date: string; category: LifeEventCategory; title: string; side: LifeEventSide; startTime: string; endTime: string; note?: string };
...
const defaultLifeEventCategories = ["吃饭", "睡觉", "健身", "通勤", "洗漱", "家务", "休息", "其他"];
const lifeEventIcon = (category: LifeEventCategory) => ({ "吃饭": "餐", "睡觉": "眠", "健身": "练", "通勤": "行", "洗漱": "洗", "家务": "家", "休息": "休", "其他": "记" }[category] || category.trim().slice(0, 1) || "记");
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, setSessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; timer: TimerShared }) {
  ...
  const selectedLifeEvents = lifeEvents.filter(event => normalizedDate(event.date) === date || (studyCrossesMidnight(event.startTime, event.endTime) && shiftISODate(normalizedDate(event.date), 1) === date));
  const lifeSegments = lifeEvents.flatMap(event => clockRangeSegmentsForDate(event.date, date, event.startTime, event.endTime).map((slot, index) => ({ event, start: slot.start, end: slot.end, key: `life-${event.id}-${date}-${index}` })));
  const plannedBlocks = [
  ...
  const resetLifeForm = () => { setLifeEditingId(null); setLifeDate(date); setLifeCategory("吃饭"); setLifeTitle(""); setLifeSide("actual"); setLifeStart("12:00"); setLifeEnd("12:30"); setLifeNote(""); };
  const editLifeEvent = (event: LifeEvent) => { setLifeEditingId(event.id); setLifeDate(normalizedDate(event.date)); setLifeCategory(event.category); setLifeTitle(event.title); setLifeSide(event.side); setLifeStart(event.startTime); setLifeEnd(event.endTime); setLifeNote(event.note || ""); };
  const saveLifeEvent = (event: FormEvent) => {
    event.preventDefault();
    const duration = studyRangeMinutes(lifeStart, lifeEnd);
    const cleanCategory = lifeCategory.trim();
    if (!cleanCategory) { window.alert("请填写日常事件类型，例如吃饭、睡觉、阅读或其他自定义类型。"); return; }
    if (!duration) { window.alert("日常事件时间范围无效；跨零点事件可以直接填写，例如 23:30–07:00。"); return; }
    const next: LifeEvent = { id: lifeEditingId || Date.now(), date: normalizedDate(lifeDate), category: cleanCategory, title: lifeTitle.trim() || cleanCategory, side: lifeSide, startTime: lifeStart, endTime: lifeEnd, note: lifeNote.trim() || undefined };
...
已经保存的日常事件不会被删除。`)) return;
  ...
  return <div className="page-stack">
    ...
    <section className="panel life-event-panel"><div className="life-event-head"><div><p className="eyebrow">DAILY LIFE</p><h2>日常事件记录</h2><span>记录吃饭、睡觉、健身、通勤、家务等；可放到时间轴左侧“预计”或右侧“实际”，不会进入月度计划表格。</span></div><strong>{selectedLifeEvents.length} 条</strong></div><form className="life-event-form" onSubmit={saveLifeEvent}><label className="life-date-field">日期<input type="date" value={lifeDate} onChange={e => setLifeDate(e.target.value)} /></label><label className="life-category-field">类型<input list="life-event-category-options" value={lifeCategory} onChange={e => setLifeCategory(e.target.value)} placeholder="可自定义，如 阅读、购物" /><datalist id="life-event-category-options">{lifeCategories.map(category => <option key={category} value={category} />)}</datalist></label><label className="life-side-field">显示位置<select value={lifeSide} onChange={e => setLifeSide(e.target.value as LifeEventSide)}><option value="planned">左侧 · 预计安排</option><option value="actual">右侧 · 实际发生</option></select></label><label className="life-title-field">事件名称<input value={lifeTitle} onChange={e => setLifeTitle(e.target.value)} placeholder={`例如：${lifeCategory === "睡觉" ? "夜间睡眠" : lifeCategory === "健身" ? "力量训练" : lifeCategory === "吃饭" ? "午饭" : "日常小事"}`} /></label><label className="life-start-field">开始时间<input type="time" value={lifeStart} onChange={e => setLifeStart(e.target.value)} /></label><label className="life-end-field">结束时间<input type="time" value={lifeEnd} onChange={e => setLifeEnd(e.target.value)} /></label><div className="life-form-hint">类型可直接输入；新类型保存后会自动加入类型选项。时间支持跨零点，例如 23:30–07:00。</div><label className="life-note-field">备注<input value={lifeNote} onChange={e => setLifeNote(e.target.value)} placeholder="可选，例如：公司食堂、跑步5km" /></label><div className="life-form-actions">{lifeEditingId && <button className="soft-button" type="button" onClick={resetLifeForm}>取消修改</button>}<button className="primary-button" type="submit">{lifeEditingId ? "保存修改" : "＋ 添加到时间轴"}</button></div></form><div className="life-category-manager"><span>类型选项<small>拖动 ≡ 可调整顺序</small></span><div>{lifeCategories.map(category => <span className="life-category-chip" key={category} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={event => { event.preventDefault(); reorderLifeCategory(event.dataTransfer.getData("text/plain"), category); }}><i className="life-category-drag" draggable title={`拖动“${category}”调整顺序`} aria-label={`拖动类型${category}`} onDragStart={event => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", category); }}>≡</i><b>{category}</b><button type="button" title={`修改${category}`} aria-label={`修改类型${category}`} onClick={() => renameLifeCategory(category)}>✎</button><button type="button" className="delete" title={`删除${category}`} aria-label={`删除类型${category}`} onClick={() => deleteLifeCategory(category)}>×</button></span>)}</div></div>{selectedLifeEvents.length ? <div className="life-event-list">{selectedLifeEvents.slice().sort((a, b) => (timeMinutes(a.startTime) || 0) - (timeMinutes(b.startTime) || 0)).map(event => <article key={event.id}><span className="life-event-icon">{lifeEventIcon(event.category)}</span><div><strong>{event.title}</strong><small>{normalizedDate(event.date) !== date ? `${formatShortDate(event.date)}起 · ` : ""}{event.category} · {formatStudyRange(event.startTime, event.endTime)} · {event.side === "planned" ? "左侧预计" : "右侧实际"}{event.note ? ` · ${event.note}` : ""}</small></div><button type="button" onClick={() => editLifeEvent(event)}>修改</button><button className="life-delete" type="button" onClick={() => { if (window.confirm(`删除“${event.title}”这条日常事件吗？`)) { setLifeEvents(current => current.filter(item => item.id !== event.id)); if (lifeEditingId === event.id) resetLifeForm(); } }}>删除</button></article>)}</div> : <div className="all-scheduled">{date} 还没有日常事件记录。</div>}</section>
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button className="timeline-reset" onClick={() => setTimelineZoom(100)}>↺ 恢复标准</button></div><div className="timeline-head"><span>预计任务 / 日常安排</span><b>时间</b><span>实际学习 / 日常记录</span></div><div className="daily-timeline timeline-full-day" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{plannedLayout.map(({ item, lane, laneCount }) => { const study = item.entryType === "study" ? item.item : null; const life = item.entryType === "life" ? item.lifeEvent : null; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : " · 点击修改预计时间段"}`} key={item.key} className={`${life ? "life-event-block" : ""} ${study?.done ? "done" : ""} ${study?.state?.canceled ? "canceled" : ""} ${(study?.state?.delayCount || 0) >= 2 ? "repeated-delay" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{study ? <button className="timeline-check" onClick={event => { event.stopPropagation(); togglePlanned(study); }}>{study.done ? "✓" : ""}</button> : <span className="timeline-life-dot">{life ? lifeEventIcon(life.category) : "记"}</span>}<button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改预计时间段"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (study) beginEdit(study); }}>✎</button><div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); else if (study) beginEdit(study); }}><span className="timeline-time">{item.start}–{item.end}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : study?.state?.canceled ? " · 今日取消" : study?.state?.delayCount ? ` · 已推迟${study.state.delayCount}次` : ""}</small></div></article>; })}</div><div className="timeline-axis">{Array.from({ length: 25 }, (_, i) => i).map((hour, index) => <div key={hour} style={{ top: `${index * 100 / 24}%` }}><span>{pad(hour)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actualLayout.map(({ item, lane, laneCount }) => { const life = item.entryType === "life" ? item.lifeEvent : null; const studySession = item.entryType === "study" ? item.session : null; const editableStudy = studySession && studySession.id >= 0; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : editableStudy ? " · 点击铅笔修改实际学习记录" : ""}`} key={item.key} className={`${life ? "life-event-block" : "actual-study-block"} ${item.running ? "running" : ""} ${item.paused ? "paused" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{life && <span className="timeline-life-dot">{lifeEventIcon(life.category)}</span>}{(life || editableStudy) && <button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改实际学习时间与备注"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (studySession) beginActualEdit(studySession); }}>✎</button>}<div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); }}><span className="timeline-time">{item.start}–{item.end}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : ""}</small></div></article>; })}</div></div></section>

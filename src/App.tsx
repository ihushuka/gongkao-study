...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, setSessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; timer: TimerShared }) {
  ...
  const actualLayout = layoutTimelineEntries(actualBlocks, x => x.start, x => {
...
已经保存的日常事件不会被删除。`)) return;
  ...
  return <div className="page-stack">
    ...
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><label className="timeline-start-control"><span>起始时间</span><select value={safeTimelineStartHour} onChange={e => setTimelineStartHour(Number(e.target.value))}>{Array.from({ length: 13 }, (_, hour) => <option key={hour} value={hour}>{pad(hour)}:00</option>)}</select></label><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button className="timeline-reset" onClick={() => { setTimelineZoom(100); setTimelineStartHour(7); }}>↺ 恢复标准</button></div><div className="timeline-head"><span>预计任务 / 日常安排</span><b>时间</b><span>实际学习 / 日常记录</span></div><div className="daily-timeline timeline-full-day" style={{ height: `${timelinePixelHeight}px` }}><div className="timeline-side planned-side">{plannedLayout.filter(({ item }) => timelineVisible(item.start, item.end)).map(({ item, lane, laneCount }) => { const study = item.entryType === "study" ? item.item : null; const life = item.entryType === "life" ? item.lifeEvent : null; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : " · 点击修改预计时间段"}`} key={item.key} className={`${life ? `life-event-block ${lifeEventIsPoint(life) ? "point-event-block" : ""}` : ""} ${study?.done ? "done" : ""} ${study?.state?.canceled ? "canceled" : ""} ${(study?.state?.delayCount || 0) >= 2 ? "repeated-delay" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{study ? <button className="timeline-check" onClick={event => { event.stopPropagation(); togglePlanned(study); }}>{study.done ? "✓" : ""}</button> : <span className="timeline-life-dot">{life ? lifeEventIcon(life.category) : "记"}</span>}<button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改预计时间段"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (study) beginEdit(study); }}>✎</button><div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); else if (study) beginEdit(study); }}><span className="timeline-time">{life && lifeEventIsPoint(life) ? item.start : `${item.start}–${item.end}`}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : study?.state?.canceled ? " · 今日取消" : study?.state?.delayCount ? ` · 已推迟${study.state.delayCount}次` : ""}</small></div></article>; })}</div><div className="timeline-axis">{timelineHours.map((hour, index) => <div key={hour} style={{ top: `${index * 100 / Math.max(1, timelineHours.length - 1)}%` }}><span>{pad(hour)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actualLayout.filter(({ item }) => timelineVisible(item.start, item.end)).map(({ item, lane, laneCount }) => { const life = item.entryType === "life" ? item.lifeEvent : null; const studySession = item.entryType === "study" ? item.session : null; const editableStudy = studySession && studySession.id >= 0; const shortActualStudy = Boolean(studySession) && timelineDuration(item.start, item.end) < 20; const shortTopPx = Math.max(0, Math.min(Math.max(0, timelinePixelHeight - 38), timelineTop(item.start) / 100 * timelinePixelHeight)); const positionStyle = shortActualStudy ? { top: `${shortTopPx}px`, height: "36px", ...laneStyle(lane, laneCount) } : { top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : editableStudy ? " · 点击铅笔修改实际学习记录" : ""}`} key={item.key} className={`${life ? `life-event-block ${lifeEventIsPoint(life) ? "point-event-block" : ""}` : `actual-study-block ${shortActualStudy ? "short-actual-study" : ""}`} ${item.running ? "running" : ""} ${item.paused ? "paused" : ""} ${shortActualStudy ? "" : timelineDensity(item.start, item.end)}`} style={positionStyle}>{life && <span className="timeline-life-dot">{lifeEventIcon(life.category)}</span>}{(life || editableStudy) && <button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改实际学习时间与备注"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (studySession) beginActualEdit(studySession); }}>✎</button>}<div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); }}><span className="timeline-time">{life && lifeEventIsPoint(life) ? item.start : `${item.start}–${item.end}`}</span><strong>{item.title}</strong><small>{shortActualStudy ? `${timelineDuration(item.start, item.end)} 分钟` : `${item.label}${life?.note ? ` · ${life.note}` : ""}`}</small></div></article>; })}</div></div><p className="timeline-range-note">当前显示 {pad(safeTimelineStartHour)}:00–24:00；更早的记录仍保留，只是在时间轴中隐藏。</p></section>

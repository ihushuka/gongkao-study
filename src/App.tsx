...
已经保存的日常事件不会被删除。`)) return;
  ...
  const denseGroupTopPx = (group: DenseActualGroup) => Math.max(0, Math.min(Math.max(0, timelinePixelHeight - 42), timelineTop(clockFromMinute(group.startMinute)) / 100 * timelinePixelHeight));
  ...
  denseActualGroups.forEach(group => {
    ...
    const displayTop = Math.min(Math.max(0, timelinePixelHeight - 42), Math.max(rawTop, denseLastBottom + 4));
    denseGroupDisplayTop.set(group.id, displayTop);
    denseLastBottom = displayTop + 42;
  });
            ...
            {timelineMode === "smart" && denseActualGroups.map(group => {
              ...
              return <div className={`dense-actual-group ${expanded ? "expanded" : ""}`} key={group.id} style={{ top: `${denseGroupDisplayTop.get(group.id) ?? denseGroupTopPx(group)}px` }}>
                <button type="button" className="dense-group-summary" onClick={() => setExpandedDenseGroup(expanded ? null : group.id)} aria-expanded={expanded}>
                  ...
                  <span className="dense-group-copy"><strong>{clockFromMinute(group.startMinute)}–{clockFromMinute(group.endMinute)} · {group.items.length} 项记录</strong><small>{expanded ? "点击收起局部详情" : "该时段记录较密，点击展开查看"}</small></span>
                  <b>{expanded ? "收起" : "展开"}</b>
                </button>
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer, moduleOptions, setModuleOptions }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>> }) {
  ...
  const studyModuleDefaults = ["focus", "week", "distribution", "noise", "heatmap", "manual", "recent"] as const;
  ...
  const studyModuleLabels: Record<StudyModuleId, string> = { focus: "当前专注", week: "近7天学习时长", distribution: "模块投入分布", noise: "环境声音", heatmap: "月历学习时长", manual: "补录学习时长", recent: "最近记录" };
  ...
  const moduleStats = studyModules.map(m => ({ module: m, seconds: sessions.filter(s => s.module === m).reduce((sum, s) => sum + s.seconds, 0) + (timer.timerOn && timer.timerModule === m ? timer.activeSeconds : 0) }));
  ...
  const studyModuleContent = (id: StudyModuleId) => {
    ...
    if (id === "distribution") return <div className="panel"><PanelTitle title="模块投入分布" /><div className="horizontal-bars">{moduleStats.map(x => <div key={x.module}><span>{x.module}</span><div><i style={{ width: `${x.seconds / maxModule * 100}%` }} /></div><strong>{Math.round(x.seconds / 360) / 10}h</strong></div>)}</div></div>;
...
function PracticeView({ practices, setPractices, flash, moduleOptions, setModuleOptions, timer, taskPresets, setTaskPresets }: { practices: Practice[]; setPractices: Dispatch<SetStateAction<Practice[]>>; flash: (x: string) => void; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>>; timer: TimerShared; taskPresets: StudyTaskPreset[]; setTaskPresets: Dispatch<SetStateAction<StudyTaskPreset[]>> }) {
  ...
  const trendSeries = allModules.map((moduleName, seriesIndex) => {
    const points = trendDates.map((day, dayIndex) => {
      const rows = currentRows.filter(row => row.module === moduleName && normalizedDate(row.date) === day);
      if (!rows.length) return null;
      const pct = Math.round(rate(rows) * 10) / 10;
      return { x: 50 + dayIndex * 600 / Math.max(1, range - 1), y: 178 - pct / 100 * 138, pct, day };
    }).filter((point): point is { x: number; y: number; pct: number; day: string } => point !== null);
  ...
  return <div className="page-stack"><section className="page-intro"><div><p className="eyebrow">ACCURACY TRACKER</p><h2>看见每一次进步的轨迹</h2><p>按模块、学习任务、课时与来源拆分；各科正确率共用同一条日期时间轴。</p></div></section>
    <section className="panel option-manager"><div className="option-column"><div><h2>其他练习来源管理</h2><p>学习任务会自动出现在“记录一组练习”的来源中；这里仅管理夸夸刷、真题、题库等独立来源。</p></div><div className="option-list">{sourceOptions.map((x, i) => <div key={`${x}-${i}`}><input defaultValue={x} onBlur={e => renameOption("source", i, e.target.value)} /><button onClick={() => deleteOption("source", i)}>删除</button></div>)}</div><div className="option-add"><input value={newSource} onChange={e => setNewSource(e.target.value)} placeholder="新增独立来源，如：小程序模考" /><button onClick={() => addOption("source")}>添加</button></div></div><div className="option-column"><div><h2>学习模块管理</h2><p>新增、改名、删除或拖动排序都会立即同步到学习计时、学习任务和正确率统计中。</p></div><div className="option-list module-option-list">{moduleOptions.map((x, i) => <div className="module-option-row" key={`${x}-${i}`} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={event => { event.preventDefault(); reorderModuleOption(event.dataTransfer.getData("text/plain"), x); }}><i className="option-drag-handle" draggable title={`拖动“${x}”调整顺序`} aria-label={`拖动学习模块${x}`} onDragStart={event => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", x); }}>≡</i><input defaultValue={x} onBlur={e => renameOption("module", i, e.target.value)} /><button onClick={() => deleteOption("module", i)}>删除</button></div>)}</div><div className="option-add"><input value={newModule} onChange={e => setNewModule(e.target.value)} placeholder="新增模块，如：政治理论" /><button onClick={() => addOption("module")}>添加</button></div></div></section>

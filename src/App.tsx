...
const sessionDisplayModule = (session: StudySession, tasks: Task[], routines: DailyRoutine[]) => resolveLinkedTask(session, tasks, routines)?.subject || session.module;
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; timer: TimerShared }) {
  ...
  return <div className="page-stack"><section className="page-intro daily-intro"><div><p className="eyebrow">DAILY TIMELINE</p><h2>计划时间与真实投入并排看</h2><p>左边是预计安排，右边由学习计时自动生成实际完成时段。</p><span className="sync-badge">✓ 已与月度计划实时同步</span></div><input type="date" value={date} onChange={e => setDate(e.target.value)} /></section>
    ...
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button className="timeline-reset" onClick={() => setTimelineZoom(100)}>↺ 恢复标准</button></div><div className="timeline-head"><span>预计任务安排</span><b>时间</b><span>实际学习记录</span></div><div className="daily-timeline" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{plannedLayout.map(({ item, lane, laneCount }) => <article title={`${item.title} · ${item.start}–${item.end} · ${item.subject}`} key={`${item.kind}${item.id}`} className={`${item.done ? "done" : ""} ${item.state?.canceled ? "canceled" : ""} ${(item.state?.delayCount || 0) >= 2 ? "repeated-delay" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}><button className="timeline-check" onClick={() => togglePlanned(item)}>{item.done ? "✓" : ""}</button><div className="timeline-inline" onClick={() => beginEdit(item)}><span className="timeline-time">{item.start}–{item.end}</span><strong>{item.title}</strong><small>{item.subject}{item.state?.canceled ? " · 今日取消" : item.state?.delayCount ? ` · 已推迟${item.state.delayCount}次` : ""}</small></div></article>)}</div><div className="timeline-axis">{Array.from({ length: 19 }, (_, i) => i + 6).map((h, index) => <div key={h} style={{ top: `${index * 100 / 18}%` }}><span>{pad(h)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actualLayout.map(({ item, lane, laneCount }) => <article title={`${item.title || item.module} · ${item.startTime}–${item.endTime || "进行中"}`} key={item.id} className={`${item.running ? "running" : ""} ${timelineDensity(item.startTime, item.endTime)}`} style={{ top: `${timelineTop(item.startTime)}%`, height: `${timelineHeight(item.startTime, item.endTime)}%`, ...laneStyle(lane, laneCount) }}><div className="timeline-inline"><span className="timeline-time">{item.startTime}–{item.endTime || "进行中"}</span><strong>{item.title || item.module}</strong><small>{formatHours(item.seconds)}</small></div></article>)}</div></div></section>
  </div>;
}
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared }) {
  ...
  const addManual = (e: FormEvent) => {
    ...
    const row = { date, module, title: recordTitle.trim(), source: recordSource.trim() || undefined, linkedTaskKey: recordTaskKey || undefined, seconds: finalMinutes * 60, startTime, endTime };
    if (editingId !== null) { setSessions(sessions.map(s => s.id === editingId ? { ...s, ...row } : s)); setEditingId(null); flash("学习记录已修改，并同步到实际时间轴"); }
    else { setSessions([...sessions, { id: Date.now(), ...row }]); flash("学习记录已补录，并同步到实际时间轴"); }

...
type Practice = { id: number; date: string; source: string; module: string; correct: number; total: number; minutes: number; presetId?: number };
...
已经保存的日常事件不会被删除。`)) return;
  ...
  type ActualStudyBlock = Extract<ActualTimelineBlock, { entryType: "study" }>;
  type DenseActualGroup = { id: string; items: ActualStudyBlock[]; startMinute: number; endMinute: number };
  const shortStudyBlocks = actualBlocks.filter((item): item is ActualStudyBlock => item.entryType === "study" && timelineDuration(item.start, item.end) < 20 && timelineVisible(item.start, item.end)).sort((a, b) => (timeMinutes(a.start) || 0) - (timeMinutes(b.start) || 0));
  const denseActualGroups: DenseActualGroup[] = [];
  let denseCandidate: ActualStudyBlock[] = [], denseWindowStart = -1;
  ...
  shortStudyBlocks.forEach(item => {
    ...
    if (startMinute - denseWindowStart <= 30) { denseCandidate.push(item); return; }
  ...
  return <div className="page-stack">
    ...
    <section className="panel daily-timeline-panel">
      <div className="timeline-view-toolbar">
        ...
        {timelineMode === "smart" && <span className="timeline-mode-hint">短记录改用节点卡片；半小时内 ≥3 条短记录自动合并{denseActualGroups.length ? ` · 今日识别 ${denseActualGroups.length} 个密集时段` : ""}</span>}
          ...
          <div className="timeline-side actual-side">
            ...
            {timelineMode === "smart" && denseActualGroups.map(group => {
              ...
              return <div className={`dense-actual-group ${expanded ? "expanded" : ""}`} key={group.id} style={{ top: `${denseGroupDisplayTop.get(group.id) ?? denseGroupTopPx(group)}px` }}>
                ...
                {expanded && <div className="dense-group-detail">{group.items.map(member => { const session = member.session; const editable = session.id >= 0; return <div className="dense-detail-row" key={member.key}><span>{member.start}–{member.end}</span><strong title={member.title}>{member.title}</strong><em>{timelineDuration(member.start, member.end)}m</em>{editable ? <button type="button" title="修改实际学习记录" onClick={() => beginActualEdit(session)}>✎</button> : <i>进行中</i>}</div>; })}</div>}
              </div>;
            })}
          </div>
        </div>
        <p className="timeline-range-note">当前显示 {pad(safeTimelineStartHour)}:00–24:00；更早的记录仍保留，只是在时间轴中隐藏。{timelineMode === "smart" ? " 密集短记录会自动合并，可点击局部展开。" : ""}</p>
      </>}
...
function PracticeView({ practices, setPractices, flash, moduleOptions, setModuleOptions, timer, taskPresets, setTaskPresets }: { practices: Practice[]; setPractices: Dispatch<SetStateAction<Practice[]>>; flash: (x: string) => void; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>>; timer: TimerShared; taskPresets: StudyTaskPreset[]; setTaskPresets: Dispatch<SetStateAction<StudyTaskPreset[]>> }) {
  const orderedTaskPresets = [...taskPresets].sort((a, b) => (a.order ?? a.id) - (b.order ?? b.id));
  const [range, setRange] = useState<7 | 30>(7), [date, setDate] = useStoredState<string>("shore-practice-selected-date", localISO()), [source, setSource] = useState(""), [linkedPresetId, setLinkedPresetId] = useState<number | null>(null), [module, setModule] = useState(moduleOptions[0] || "资料分析"), [correct, setCorrect] = useState(18), [total, setTotal] = useState(20), [minutes, setMinutes] = useState(24);
  ...
  const choosePracticeSource = (value: string) => {
    if (value.startsWith("task:")) {
      ...
      if (!preset) return;
      setLinkedPresetId(preset.id); setSource(preset.title); setModule(preset.subject);
      return;
    }
    if (value.startsWith("source:")) { setLinkedPresetId(null); setSource(value.slice(7)); return; }
    setLinkedPresetId(null); setSource("");
  };
  ...
  const add = (e: FormEvent) => { e.preventDefault(); if (!source.trim()) { flash("请选择练习来源或关联学习任务"); return; } if (correct > total) { flash("正确题数不能大于总题数，请检查后再保存"); return; } if (correct < 0 || total <= 0) { flash("题数必须是有效的正数"); return; } const linked = linkedPresetId !== null ? taskPresets.find(item => item.id === linkedPresetId) : undefined; const row = { date, source: linked?.title || source, module: linked?.subject || module, correct, total, minutes, presetId: linked?.id }; if (editingId !== null) { setPractices(practices.map(p => p.id === editingId ? { ...p, ...row } : p)); setEditingId(null); flash(linked ? "练习记录已修改，并保持与学习任务同步" : "练习记录已修改"); } else { setPractices([...practices, { id: Date.now(), ...row }]); flash(linked ? "练习成绩已记录，并已关联学习任务" : "练习成绩已记录"); } };
  const editPractice = (p: Practice) => { const linked = p.presetId !== undefined ? taskPresets.find(item => item.id === p.presetId) : undefined; setEditingId(p.id); setDate(normalizedDate(p.date)); setLinkedPresetId(linked?.id ?? null); setSource(linked?.title || p.source); setModule(linked?.subject || p.module); setCorrect(p.correct); setTotal(p.total); setMinutes(p.minutes); };
  const cancelPracticeEdit = () => { setEditingId(null); setLinkedPresetId(null); setSource(""); };

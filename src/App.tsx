...
type Task = { id: number; title: string; subject: string; minutes: number; done: boolean; date?: string; plannedStart?: string; plannedEnd?: string; plannedSlots?: TimeSlot[]; supervision?: SupervisionState; presetId?: number; lessonNumber?: number };
type StudyTaskPreset = { id: number; title: string; subject: string; totalLessons: number; order?: number };
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, setSessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; timer: TimerShared }) {
  ...
  const actualLayout = layoutTimelineEntries(actualBlocks, x => x.start, x => x.end);
...
已经保存的日常事件不会被删除。`)) return;
  ...
  const timelineTop = (time?: string) => Math.max(0, Math.min(100, ((timeMinutes(time) ?? 0) / 1440) * 100));
  ...
  const timelineHeight = (start?: string, end?: string) => (timelineDuration(start, end) / 1440) * 100;
  ...
  return <div className="page-stack">
    ...
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button className="timeline-reset" onClick={() => setTimelineZoom(100)}>↺ 恢复标准</button></div><div className="timeline-head"><span>预计任务 / 日常安排</span><b>时间</b><span>实际学习 / 日常记录</span></div><div className="daily-timeline timeline-full-day" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{plannedLayout.map(({ item, lane, laneCount }) => { const study = item.entryType === "study" ? item.item : null; const life = item.entryType === "life" ? item.lifeEvent : null; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : " · 点击修改预计时间段"}`} key={item.key} className={`${life ? `life-event-block ${lifeEventIsPoint(life) ? "point-event-block" : ""}` : ""} ${study?.done ? "done" : ""} ${study?.state?.canceled ? "canceled" : ""} ${(study?.state?.delayCount || 0) >= 2 ? "repeated-delay" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{study ? <button className="timeline-check" onClick={event => { event.stopPropagation(); togglePlanned(study); }}>{study.done ? "✓" : ""}</button> : <span className="timeline-life-dot">{life ? lifeEventIcon(life.category) : "记"}</span>}<button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改预计时间段"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (study) beginEdit(study); }}>✎</button><div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); else if (study) beginEdit(study); }}><span className="timeline-time">{life && lifeEventIsPoint(life) ? item.start : `${item.start}–${item.end}`}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : study?.state?.canceled ? " · 今日取消" : study?.state?.delayCount ? ` · 已推迟${study.state.delayCount}次` : ""}</small></div></article>; })}</div><div className="timeline-axis">{Array.from({ length: 25 }, (_, i) => i).map((hour, index) => <div key={hour} style={{ top: `${index * 100 / 24}%` }}><span>{pad(hour)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actualLayout.map(({ item, lane, laneCount }) => { const life = item.entryType === "life" ? item.lifeEvent : null; const studySession = item.entryType === "study" ? item.session : null; const editableStudy = studySession && studySession.id >= 0; return <article title={`${item.title} · ${item.start}–${item.end}${life ? " · 日常事件" : editableStudy ? " · 点击铅笔修改实际学习记录" : ""}`} key={item.key} className={`${life ? `life-event-block ${lifeEventIsPoint(life) ? "point-event-block" : ""}` : "actual-study-block"} ${item.running ? "running" : ""} ${item.paused ? "paused" : ""} ${timelineDensity(item.start, item.end)}`} style={{ top: `${timelineTop(item.start)}%`, height: `${timelineHeight(item.start, item.end)}%`, ...laneStyle(lane, laneCount) }}>{life && <span className="timeline-life-dot">{lifeEventIcon(life.category)}</span>}{(life || editableStudy) && <button className="timeline-edit-trigger" title={life ? "修改日常事件" : "修改实际学习时间与备注"} aria-label={`修改${item.title}`} onClick={event => { event.stopPropagation(); if (life) editLifeEvent(life); else if (studySession) beginActualEdit(studySession); }}>✎</button>}<div className="timeline-inline" onClick={() => { if (life) editLifeEvent(life); }}><span className="timeline-time">{life && lifeEventIsPoint(life) ? item.start : `${item.start}–${item.end}`}</span><strong>{item.title}</strong><small>{item.label}{life?.note ? ` · ${life.note}` : ""}</small></div></article>; })}</div></div></section>
...
function StudyTaskManager({ presets, setPresets, tasks, setTasks, moduleOptions, flash }: { presets: StudyTaskPreset[]; setPresets: Dispatch<SetStateAction<StudyTaskPreset[]>>; tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; moduleOptions: string[]; flash: (x: string) => void }) {
  ...
  const savePreset = (event: FormEvent) => {
    event.preventDefault(); const clean = title.trim(), total = Math.max(1, Math.round(totalLessons)); if (!clean) { flash("请填写学习任务名称"); return; }
    if (editingId !== null) {
      setPresets(current => current.map(item => item.id === editingId ? { ...item, title: clean, subject, totalLessons: total } : item));
      setEditingId(null); flash("学习任务已更新");
    } else {
      ...
      setPresets(current => [...current, { id: Date.now(), title: clean, subject, totalLessons: total, order: nextOrder }]);
      flash("学习任务已加入预设库");
    }
    setTitle(""); setTotalLessons(12);
  };
  const editPreset = (preset: StudyTaskPreset) => { setEditingId(preset.id); setTitle(preset.title); setSubject(preset.subject); setTotalLessons(preset.totalLessons); };
  const deletePreset = (preset: StudyTaskPreset) => {
    if (!window.confirm(`确定删除预设任务“${preset.title}”吗？\n已经加入月度计划的任务不会被删除。`)) return;
    setPresets(current => current.filter(item => item.id !== preset.id));
    setTasks(current => current.map(task => task.presetId === preset.id ? { ...task, presetId: undefined, lessonNumber: undefined } : task));
    if (editingId === preset.id) { setEditingId(null); setTitle(""); setTotalLessons(12); }
  ...
  return <div className="page-stack task-library-page">
    <section className="page-intro"><div><p className="eyebrow">STUDY TASK LIBRARY</p><h2>学习任务管理</h2><p>把课程、题库或长期学习项目预设好；之后在“月度计划 → 添加单日任务”中可以直接选择，并按课时加入当天。</p></div></section>
    <section className="task-library-layout">
      <form className="panel form-card task-preset-form" onSubmit={savePreset}><PanelTitle title={editingId === null ? "新增预设任务" : "修改预设任务"} /><label>任务名称<input value={title} onChange={e => setTitle(e.target.value)} placeholder="例如：张弓十二箭" /></label><div className="form-grid"><label>学习模块<select value={subject} onChange={e => setSubject(e.target.value)}>{moduleOptions.map(item => <option key={item}>{item}</option>)}</select></label><label>总课时<input type="number" min="1" max="999" value={totalLessons} onChange={e => setTotalLessons(Math.max(1, Number(e.target.value) || 1))} /></label></div><button className="primary-button wide">{editingId === null ? "加入任务库" : "保存修改"}</button>{editingId !== null && <button type="button" className="soft-button wide cancel-edit" onClick={() => { setEditingId(null); setTitle(""); setTotalLessons(12); }}>取消修改</button>}</form>
      <section className="panel task-preset-list-panel"><div className="panel-title"><div><h2>预设任务</h2><p>拖动左侧“≡”可以调整在月度计划选择框中的顺序。</p></div><span>{ordered.length} 项</span></div>{ordered.length ? <div className="task-preset-list">{ordered.map(preset => { const linked = tasks.filter(task => task.presetId === preset.id); const scheduledLessons = new Set(linked.map(task => task.lessonNumber).filter((value): value is number => Number.isFinite(value))).size; const completedLessons = new Set(linked.filter(task => task.done).map(task => task.lessonNumber).filter((value): value is number => Number.isFinite(value))).size; const percent = preset.totalLessons ? Math.min(100, Math.round(completedLessons / preset.totalLessons * 100)) : 0; return <article className="task-preset-card" key={preset.id} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); movePreset(Number(event.dataTransfer.getData("text/plain")), preset.id); }}><i className="task-preset-drag" draggable title="拖动排序" onDragStart={event => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", String(preset.id)); }}>≡</i><div className="task-preset-main"><div><strong>{preset.title}</strong><span>{preset.subject}</span></div><p>总课时 {preset.totalLessons} · 已安排 {Math.min(scheduledLessons, preset.totalLessons)} · 已完成 {Math.min(completedLessons, preset.totalLessons)}</p><div className="progress-track"><i style={{ width: `${percent}%` }} /></div></div><div className="task-preset-actions"><button onClick={() => editPreset(preset)}>修改</button><button onClick={() => deletePreset(preset)}>删除</button></div></article>; })}</div> : <div className="all-scheduled">还没有预设学习任务。可以先添加“张弓十二箭 / 言语理解 / 12课时”这类长期任务。</div>}</section>
    </section>
  </div>;
}
...
function Plan({ tasks, setTasks, routines, setRoutines, flash, moduleOptions, taskPresets }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; flash: (x: string) => void; moduleOptions: string[]; taskPresets: StudyTaskPreset[] }) {
  ...
  const [selectedPresetId, setSelectedPresetId] = useState(""), [presetLesson, setPresetLesson] = useState(1);
  ...
  const nextPresetLesson = (preset: StudyTaskPreset) => { const used = new Set(tasks.filter(task => task.presetId === preset.id && Number.isFinite(task.lessonNumber)).map(task => task.lessonNumber as number)); for (let lesson = 1; lesson <= preset.totalLessons; lesson += 1) if (!used.has(lesson)) return lesson; return preset.totalLessons; };
  const chooseTaskPreset = (rawId: string) => { setSelectedPresetId(rawId); if (!rawId) return; const preset = orderedTaskPresets.find(item => String(item.id) === rawId); if (!preset) return; const lesson = nextPresetLesson(preset); setPresetLesson(lesson); setSubject(preset.subject); setTitle(`${preset.title} 第${lesson}课`); };
  const changePresetLesson = (value: number) => { if (!selectedPreset) return; const lesson = Math.min(selectedPreset.totalLessons, Math.max(1, Math.round(value || 1))); setPresetLesson(lesson); setTitle(`${selectedPreset.title} 第${lesson}课`); setSubject(selectedPreset.subject); };
  useEffect(() => {
  ...
  const add = (e: FormEvent) => { e.preventDefault(); if (!title.trim()) return; const slots = plannedSlots.map(slot => ({ ...slot })).sort((a, b) => (timeMinutes(a.start) || 0) - (timeMinutes(b.start) || 0)); if (slots.length < 1 || slots.length > 3 || slots.some(slot => !validTimeRange(slot.start, slot.end))) { flash("请设置1至3个有效时间段，每段结束时间必须晚于开始时间"); return; } if (slots.some((slot, index) => index > 0 && (timeMinutes(slot.start) || 0) < (timeMinutes(slots[index - 1].end) || 0))) { flash("多个时间段不能互相重叠"); return; } const totalMinutes = plannedSlotsMinutes(slots); const duplicate = tasks.some(t => normalizedDate(t.date) === date && t.title.trim() === title.trim()); if (duplicate && !window.confirm("同一天已经存在完全相同的任务，仍要重复添加吗？")) return; setTasks([...tasks, { id: Date.now(), title: title.trim(), subject, minutes: totalMinutes, date, done: false, plannedStart: slots[0].start, plannedEnd: slots[slots.length - 1].end, plannedSlots: slots, presetId: selectedPreset?.id, lessonNumber: selectedPreset ? presetLesson : undefined }]); setTitle(""); setSelectedPresetId(""); setPresetLesson(1); setMinutes(totalMinutes); setMinutesInput(String(totalMinutes)); setMonth(date.slice(0, 7)); flash(selectedPreset ? `“${selectedPreset.title} 第${presetLesson}课”已加入对应日期` : "任务已加入对应日期"); };

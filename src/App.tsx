...
const presetTaskTitle = (preset: Pick<StudyTaskPreset, "title">, lessons: number[]) => lessons.length ? `${preset.title} 第${formatLessonSelection(lessons)}课` : preset.title;
...
function StudyTaskManager({ presets, setPresets, tasks, setTasks, moduleOptions, flash }: { presets: StudyTaskPreset[]; setPresets: Dispatch<SetStateAction<StudyTaskPreset[]>>; tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; moduleOptions: string[]; flash: (x: string) => void }) {
  ...
  const markHistoryCompletedUndated = () => {
    ...
    setHistorySelection([]);
    flash(`已将第${formatLessonSelection(selectable)}课标记为已完成，日期可以以后再补`);
  };
  ...
  const savePreset = (event: FormEvent) => {
    ...
    if (editingId !== null) {
      ...
      setTasks(current => current.map(task => task.presetId === editingId && !task.historyPresetRecord ? { ...task, subject, title: taskLessonNumbers(task).length ? presetTaskTitle({ title: clean }, taskLessonNumbers(task)) : task.title } : task));
      ...
      setTitle(""); setTotalLessons(12); setHistorySelection([]); setHistoryDates({}); setLegacyUndated([]); flash("学习任务已加入预设库；已完成课时已保存，有日期的记录已同步月历");
    }
  };
...
function PracticeView({ practices, setPractices, flash, moduleOptions, setModuleOptions, timer, taskPresets, setTaskPresets }: { practices: Practice[]; setPractices: Dispatch<SetStateAction<Practice[]>>; flash: (x: string) => void; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>>; timer: TimerShared; taskPresets: StudyTaskPreset[]; setTaskPresets: Dispatch<SetStateAction<StudyTaskPreset[]>> }) {
  ...
  const togglePracticeLesson = (lesson: number) => {
    if (recordedLessonMeta.has(lesson) && !allowRepeatLessons) {
      flash(`第${lesson}课已经关联过刷题记录；如需再次记录，请先开启“允许重复记录”`);
      return;
    }
  ...
  const add = (e: FormEvent) => {
    ...
    if (duplicateLessons.length) { flash(`第${formatLessonSelection(duplicateLessons)}课已有刷题记录，请取消选择或开启“允许重复记录”`); return; }
  ...
  return <div className="page-stack"><section className="page-intro"><div><p className="eyebrow">ACCURACY TRACKER</p><h2>看见每一次进步的轨迹</h2><p>按模块、学习任务、课时与来源拆分；各科正确率共用同一条日期时间轴。</p></div></section>
    <section className="practice-option-grid"><div className="panel option-column practice-source-manager"><div><h2>其他练习来源管理</h2><p>学习任务会自动出现在“记录一组练习”的来源中；独立来源也可拖动 ≡ 调整下拉框顺序。本模块现在可在“界面布局”中单独拖动。</p></div><div className="option-list module-option-list">{sourceOptions.map((x, i) => <div className="module-option-row" key={`${x}-${i}`} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={event => { event.preventDefault(); reorderSourceOption(event.dataTransfer.getData("text/plain"), x); }}><i className="option-drag-handle" draggable title={`拖动“${x}”调整顺序`} aria-label={`拖动练习来源${x}`} onDragStart={event => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", x); }}>≡</i><input defaultValue={x} onBlur={e => renameOption("source", i, e.target.value)} /><button onClick={() => deleteOption("source", i)}>删除</button></div>)}</div><div className="option-add"><input value={newSource} onChange={e => setNewSource(e.target.value)} placeholder="新增独立来源，如：小程序模考" /><button onClick={() => addOption("source")}>添加</button></div></div><div className="panel option-column practice-module-manager"><div><h2>学习模块管理</h2><p>新增、改名、删除或拖动排序都会立即同步到学习计时、学习任务和正确率统计中。本模块也可独立调整页面布局。</p></div><div className="option-list module-option-list">{moduleOptions.map((x, i) => <div className="module-option-row" key={`${x}-${i}`} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={event => { event.preventDefault(); reorderModuleOption(event.dataTransfer.getData("text/plain"), x); }}><i className="option-drag-handle" draggable title={`拖动“${x}”调整顺序`} aria-label={`拖动学习模块${x}`} onDragStart={event => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", x); }}>≡</i><input defaultValue={x} onBlur={e => renameOption("module", i, e.target.value)} /><button onClick={() => deleteOption("module", i)}>删除</button></div>)}</div><div className="option-add"><input value={newModule} onChange={e => setNewModule(e.target.value)} placeholder="新增模块，如：政治理论" /><button onClick={() => addOption("module")}>添加</button></div></div></section>

...
function WhiteNoise() {
  const [playing, setPlaying] = useState(false), [kind, setKind] = useStoredState<"雨声" | "棕噪音" | "咖啡馆">("shore-noise-kind", "雨声"), [volume, setVolume] = useStoredState<number>("shore-noise-volume", 32);
  const contextRef = useRef<AudioContext | null>(null), nodeRef = useRef<ScriptProcessorNode | null>(null), gainRef = useRef<GainNode | null>(null);
  const stop = () => { nodeRef.current?.disconnect(); gainRef.current?.disconnect(); contextRef.current?.close(); nodeRef.current = null; gainRef.current = null; contextRef.current = null; setPlaying(false); };
  const start = () => { const AudioCtor = window.AudioContext || (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext; if (!AudioCtor) return; const context = new AudioCtor(), processor = context.createScriptProcessor(4096, 1, 1), gain = context.createGain(); let last = 0, pink = 0; processor.onaudioprocess = event => { const data = event.outputBuffer.getChannelData(0); for (let i = 0; i < data.length; i += 1) { const white = Math.random() * 2 - 1; if (kind === "棕噪音") { last = (last + .02 * white) / 1.02; data[i] = last * 3.2; } else if (kind === "咖啡馆") { pink = .94 * pink + .06 * white; data[i] = pink * 1.7 + (Math.random() > .997 ? white * .18 : 0); } else { last = .985 * last + .04 * white; data[i] = last * 2.2 + white * .08; } } }; gain.gain.value = volume / 100 * .45; processor.connect(gain); gain.connect(context.destination); contextRef.current = context; nodeRef.current = processor; gainRef.current = gain; setPlaying(true); };
  useEffect(() => { if (gainRef.current) gainRef.current.gain.value = volume / 100 * .45; }, [volume]);
  useEffect(() => () => { nodeRef.current?.disconnect(); contextRef.current?.close(); }, []);
  return <section className="panel white-noise"><div><p className="eyebrow">AMBIENT SOUND</p><h2>白噪音</h2><span>声音由浏览器本地生成，不需要加载音频文件。</span></div><div className="noise-controls"><select value={kind} onChange={e => { const next = e.target.value as typeof kind; if (playing) stop(); setKind(next); }}>{["雨声", "棕噪音", "咖啡馆"].map(x => <option key={x}>{x}</option>)}</select><label>音量<input type="range" min="0" max="100" value={volume} onChange={e => setVolume(Number(e.target.value))} /></label><button className={playing ? "soft-button" : "primary-button"} onClick={playing ? stop : start}>{playing ? "停止播放" : "开始播放"}</button></div></section>;
}

...
type Idiom = { id: number; word: string; meaning: string; example: string; level: "未掌握" | "模糊" | "已掌握"; source?: string; meaningHighlights?: string[]; ocrNote?: string; kind?: "成语" | "词语" };
...
const correctIdiomFromExample = (rawWord: string, leftText: string) => {
  const word = rawWord.replace(/[^㐀-鿿]/g, ""); if (word.length < 2) return { word, note: "字数异常，请人工校对" };
  if (leftText.replace(/\s+/g, "").includes(word)) return { word, note: "已与左侧例句核对" };
  ...
  const best = ranked[0]; if (best && best.distance <= 1 && (best.inExample || best.known)) return { word: best.candidate, note: `疑似错字，已由${best.inExample ? "左侧例句" : "成语词库"}校正：${rawWord} → ${best.candidate}` };
  return { word, note: commonIdioms.includes(word) ? "已通过本地成语词库核验" : "词库与例句均未确认，请重点校对" };
};
async function recognizePeopleDailyPdf(file: File, onProgress: (text: string, value: number) => void) {
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    ...
    for (const box of findGreenBoxes(canvas)) {
      ...
      entries.forEach((entry, index) => { const corrected = correctIdiomFromExample(entry.word, leftText); found.push({ id: Date.now() + found.length, word: corrected.word, meaning: entry.meaning, example: findExample(leftText, corrected.word), level: "未掌握", source, meaningHighlights: blueTexts.length ? [blueTexts[Math.min(blueTexts.length - 1, Math.floor(index * blueTexts.length / entries.length))]] : [], ocrNote: corrected.note, kind: corrected.word.length === 4 ? "成语" : "词语" }); });
    }
  }
...
async function verifyIdiomsOnline(rows: Idiom[], onProgress: (text: string, value: number) => void) {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    try {
      ...
      if (!exact?.query?.pages?.[0]?.missing) { checked.push({ ...item, ocrNote: `${item.ocrNote || "本地校验完成"}；联网词典已确认存在` }); continue; }
      ...
      if (suggestion && characterDistance(item.word, suggestion) <= 1 && item.example.includes(suggestion)) checked.push({ ...item, word: suggestion, ocrNote: `联网词典与左侧例句共同校正：${item.word} → ${suggestion}` });
      else checked.push({ ...item, ocrNote: `${item.ocrNote || ""}；联网词典未找到完全一致条目，请重点校对`.replace(/^；/, "") });
...
function DailyBriefing() {
  const [news, setNews] = useState<NewsItem[]>([]), [loading, setLoading] = useState(true), [showAnswer, setShowAnswer] = useState(false), [refreshKey, setRefreshKey] = useState(0);
  const fact = dailyKnowledge[Math.floor(new Date(`${localISO()}T12:00:00`).getTime() / 86400000) % dailyKnowledge.length];
  useEffect(() => {
    const cacheKey = `shore-news-${localISO()}`;
    try { const cached = localStorage.getItem(cacheKey); if (cached) { setNews(JSON.parse(cached)); setLoading(false); return; } } catch { /* fetch live */ }
    setLoading(true);
    const feed = "https://www.people.com.cn/rss/politics.xml";
    fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(feed)}`).then(response => response.ok ? response.json() : Promise.reject()).then(data => {
      const rows: NewsItem[] = (data.items || []).slice(0, 5).map((item: { title?: string; link?: string; pubDate?: string }) => ({ title: item.title || "人民网时政要闻", link: item.link || "https://www.people.com.cn/", source: "人民网·人民日报", date: item.pubDate?.slice(0, 10) }));
      setNews(rows); if (rows.length) localStorage.setItem(cacheKey, JSON.stringify(rows));
    }).catch(() => setNews([])).finally(() => setLoading(false));
  }, [refreshKey]);
  return <section className="daily-briefing"><div className="panel news-panel"><div className="briefing-head"><div><p className="eyebrow">DAILY NEWS</p><h2>今日要闻</h2><span>每日读取官方公开内容，点击标题查看原文。</span></div><button onClick={() => { localStorage.removeItem(`shore-news-${localISO()}`); setRefreshKey(x => x + 1); }}>刷新</button></div>{loading ? <div className="briefing-loading">正在获取今日要闻…</div> : news.length ? <div className="news-list">{news.map((item, index) => <a href={item.link} target="_blank" rel="noreferrer" key={`${item.link}-${index}`}><i>{pad(index + 1)}</i><span><strong>{item.title}</strong><small>{item.source}{item.date ? ` · ${item.date}` : ""}</small></span><b>↗</b></a>)}</div> : <div className="news-fallback"><strong>实时内容暂时未能加载</strong><p>可能是网络或跨域服务暂时不可用，可从下方官方入口查看。</p></div>}<div className="official-links"><a href="https://www.xuexi.cn/" target="_blank" rel="noreferrer">学习强国</a><a href="https://www.people.com.cn/" target="_blank" rel="noreferrer">人民网</a><a href="https://www.gov.cn/" target="_blank" rel="noreferrer">中国政府网</a></div></div><div className="panel knowledge-card"><p className="eyebrow">DAILY KNOWLEDGE</p><span className="knowledge-tag">{fact.category}常识</span><h2>{fact.question}</h2><div className={`knowledge-answer ${showAnswer ? "show" : ""}`}>{showAnswer ? fact.answer : "先在心里作答，再查看答案"}</div><button className={showAnswer ? "soft-button" : "primary-button"} onClick={() => setShowAnswer(!showAnswer)}>{showAnswer ? "收起答案" : "查看答案"}</button><small>每天自动更换一道，适合晨间快速记忆。</small></div></section>;
}
...
function StudySupervisor({ nextTask, timer, done, total, onOpen }: { nextTask: string; timer: TimerShared; done: number; total: number; onOpen: () => void }) {
  const [expanded, setExpanded] = useState(false), [awayCount, setAwayCount] = useState(0);
  ...
  return <aside className={`focus-coach supervisor ${expanded ? "expanded" : ""}`}><button className="coach-orb" onClick={() => setExpanded(!expanded)}>{timer.timerOn ? awayCount : "督"}</button>{expanded && <div><small>{timer.timerOn ? "专注契约进行中" : "今日执行监督"}</small><strong>{timer.timerOn ? timer.timerTitle : nextTask}</strong>{timer.timerOn ? <div className="supervisor-stats"><span><b>{formatClock(timer.activeSeconds)}</b>已专注</span><span><b>{awayCount}</b>离开页面</span><span><b>{done}/{total}</b>任务完成</span></div> : <p>先明确要完成的内容，再开始计时。监督模式会记录专注期间离开网页的次数。</p>}<span><button className="primary-button" onClick={() => { if (timer.timerOn) timer.toggleTimer(); else onOpen(); setExpanded(false); }}>{timer.timerOn ? "结束并保存" : "设置专注内容"}</button>{!timer.timerOn && <button className="soft-button" onClick={() => setExpanded(false)}>稍后再说</button>}</span>{"Notification" in window && Notification.permission === "default" && <button className="coach-notify" onClick={() => Notification.requestPermission()}>开启离开页面提醒</button>}</div>}</aside>;
}
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; timer: TimerShared }) {
  ...
  return <div className="page-stack"><section className="page-intro daily-intro"><div><p className="eyebrow">DAILY TIMELINE</p><h2>计划时间与真实投入并排看</h2><p>左边是预计安排，右边由学习计时自动生成实际完成时段。</p><span className="sync-badge">✓ 已与月度计划实时同步</span></div><input type="date" value={date} onChange={e => setDate(e.target.value)} /></section>
    ...
    <section className="panel daily-timeline-panel"><div className="timeline-panel-tools"><span>时间轴缩放 <b>{timelineZoom}%</b></span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button onClick={() => setTimelineZoom(100)}>恢复标准</button></div><div className="timeline-head"><span>预计任务安排</span><b>时间</b><span>实际学习记录</span></div><div className="daily-timeline" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{planned.filter(x => x.start).map(item => <article key={`${item.kind}${item.id}`} className={item.done ? "done" : ""} style={{ top: `${timelineTop(item.start)}%`, minHeight: `${timelineHeight(item.start, item.end)}%` }}><button className="timeline-check" onClick={() => togglePlanned(item)}>{item.done ? "✓" : ""}</button><div onClick={() => beginEdit(item)}><strong>{item.title}</strong><small>{item.start}–{item.end} · {item.subject}</small></div></article>)}</div><div className="timeline-axis">{Array.from({ length: 19 }, (_, i) => i + 6).map(h => <div key={h}><span>{pad(h)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actual.filter(x => x.startTime).map(item => <article key={item.id} className={item.running ? "running" : ""} style={{ top: `${timelineTop(item.startTime)}%`, minHeight: `${timelineHeight(item.startTime, item.endTime)}%` }}><div><strong>{item.title || item.module}</strong><small>{item.startTime}–{item.endTime || "进行中"} · {formatHours(item.seconds)}</small></div></article>)}</div></div></section>
  </div>;
}
...
function Plan({ tasks, setTasks, routines, setRoutines, flash }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; flash: (x: string) => void }) {
  ...
  return <div className="page-stack">
    <section className="page-intro"><div><p className="eyebrow">MONTHLY PLAN</p><h2>把月目标拆到每一天</h2><p>与每日计划实时同步；这里的新增、勾选和删除会立即反映到每日时间轴。</p></div><div className="intro-actions"><input hidden ref={planFileRef} type="file" accept=".xlsx,.xls" onChange={importPlan} /><button className="clear-month-button" onClick={clearMonth}>清除本月计划</button><button className="import-plan-button" onClick={() => planFileRef.current?.click()} disabled={importing}><span>表</span><div><strong>{importing ? "正在识别…" : "一键导入计划表"}</strong><small>自动识别日期与每日任务</small></div></button><input className="month-picker" type="month" value={month} onChange={e => setMonth(e.target.value)} /></div></section>
...
function IdiomView({ idioms, setIdioms, flash }: { idioms: Idiom[]; setIdioms: Dispatch<SetStateAction<Idiom[]>>; flash: (x: string) => void }) {
  ...
  return <div className="page-stack">{ocrBusy && <div className="ocr-lock" role="alert"><div><span>识</span><h2>正在识别 PDF</h2><p>{ocrText || "正在准备中文识别模型…"}</p><div><i style={{ width: `${ocrProgress}%` }} /></div><strong>{ocrProgress}%</strong><small>请保持当前页面打开，不要切换栏目、刷新或关闭窗口，否则本次识别可能中断。</small></div></div>}<section className="page-intro"><div><p className="eyebrow">IDIOM NOTEBOOK</p><h2>成语不是背过，是能辨清</h2><p>支持识别人民日报图片型 PDF：绿色框选成语、左侧例句与蓝色重点词义。</p></div><div className="stat-pills"><span>总数 <b>{idioms.length}</b></span><span>未掌握 <b>{idioms.filter(x => x.level !== "已掌握").length}</b></span></div></section>
    ...
    {preview.length > 0 && <section className="panel ocr-preview"><div className="panel-title"><div><h2>识别结果校对</h2><p>系统已用本地成语词库与左侧例句交叉核验；橙色提示项请重点检查。</p></div><div className="ocr-preview-actions"><button className="soft-button" onClick={cancelPreview}>取消导入并清除</button><button className="primary-button" onClick={confirmPreview}>确认导入 {preview.length} 条</button></div></div><div className="ocr-preview-grid">{preview.map((item, i) => <article key={item.id}><div className={`ocr-check-note ${item.ocrNote?.includes("未确认") || item.ocrNote?.includes("异常") ? "warning" : "ok"}`}>{item.ocrNote || "等待校验"}</div><label>来源<input value={item.source || ""} onChange={e => setPreview(preview.map((x, j) => j === i ? { ...x, source: e.target.value } : x))} /></label><label>成语<input className="idiom-word-input" value={item.word} onChange={e => setPreview(preview.map((x, j) => j === i ? { ...x, word: e.target.value, ocrNote: "已人工修改" } : x))} /></label><label>词义（蓝色重点会直接在词义中高亮）<textarea value={item.meaning} onChange={e => setPreview(preview.map((x, j) => j === i ? { ...x, meaning: e.target.value } : x))} /></label><label>需要高亮的词义<input value={(item.meaningHighlights || []).join("；")} onChange={e => setPreview(preview.map((x, j) => j === i ? { ...x, meaningHighlights: e.target.value.split("；").filter(Boolean) } : x))} /></label><label>左侧例句（成语已联动核对）<textarea value={item.example} onChange={e => setPreview(preview.map((x, j) => j === i ? { ...x, example: e.target.value } : x))} /></label><button onClick={() => setPreview(preview.filter((_, j) => j !== i))}>移除此条</button></article>)}</div></section>}
    ...
    <form className="panel inline-form" onSubmit={e => { e.preventDefault(); if (!word || !meaning) return; setIdioms([...idioms, { id: Date.now(), word, meaning, example: "", level: "未掌握", kind: word.length === 4 ? "成语" : "词语" }]); setWord(""); setMeaning(""); flash("词条已加入积累本"); }}><h3>快速添加</h3><input value={word} onChange={e => setWord(e.target.value)} placeholder="成语或词语" /><input value={meaning} onChange={e => setMeaning(e.target.value)} placeholder="词义或易错点" /><button className="primary-button">添加</button></form></div>;
}

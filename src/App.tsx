...
function Plan({ tasks, setTasks, routines, setRoutines, flash, moduleOptions }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; flash: (x: string) => void; moduleOptions: string[] }) {
  ...
  const [title, setTitle] = useState(""), [subject, setSubject] = useState(moduleOptions[0] || "资料分析"), [minutes, setMinutes] = useState(45);
  ...
  const changePlannedStart = (value: string) => { setPlannedStart(value); setPlannedEnd(clockAfterMinutes(value, minutes)); };
  const changePlannedMinutes = (value: number) => { const next = Math.max(5, Number.isFinite(value) ? value : 5); setMinutes(next); setPlannedEnd(clockAfterMinutes(plannedStart, next)); };
  const changePlannedEnd = (value: string) => { setPlannedEnd(value); if (validTimeRange(plannedStart, value)) setMinutes(Math.max(5, (timeMinutes(value) || 0) - (timeMinutes(plannedStart) || 0))); };
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer, moduleOptions, setModuleOptions }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>> }) {
  ...
  const todayChoices = taskChoicesFor(localISO());

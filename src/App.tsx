...
export default function App() {
  ...
  let page;
  if (tab === "home") page = <Dashboard tasks={tasks} setTasks={setTasks} routines={routines} setRoutines={setRoutines} practices={practices} mistakes={mistakes} idioms={idioms} examDates={examDates} setTab={setTab} timer={sharedTimer} moduleOptions={moduleOptions} />;
  ...
  else if (tab === "study") page = <StudyTime sessions={sessions} setSessions={setSessions} tasks={tasks} routines={routines} flash={flash} timer={sharedTimer} moduleOptions={moduleOptions} />;
...
function Dashboard({ tasks, setTasks, routines, setRoutines, practices, mistakes, idioms, examDates, setTab, timer, moduleOptions }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; practices: Practice[]; mistakes: Mistake[]; idioms: Idiom[]; examDates: ExamDates; setTab: Dispatch<SetStateAction<Tab>>; timer: TimerShared; moduleOptions: string[] }) {
  const todayTasks = tasks.filter(t => normalizedDate(t.date) === localISO());
  const todayRoutines = routines.filter(r => r.month === localISO().slice(0, 7) && routineApplies(r, localISO()));
  ...
  return <div className="page-stack">
    ...
    <section className="dashboard-grid"><div className="panel task-panel"><PanelTitle title="今日学习清单" action="打开时间轴" onClick={() => setTab("daily")} /><div className="progress-track"><i style={{ width: `${completion}%` }} /></div><div className="task-list">{total ? <>{todayRoutines.map(routine => { const checked = routine.completedDates.includes(localISO()); return <label className={`task-row ${checked ? "done" : ""}`} key={`r${routine.id}`}><input type="checkbox" checked={checked} onChange={() => setRoutines(routines.map(x => x.id === routine.id ? { ...x, completedDates: checked ? x.completedDates.filter(d => d !== localISO()) : [...x.completedDates, localISO()] } : x))} /><span className="custom-check">✓</span><span className="task-copy"><strong>{routine.title}</strong><small>每日任务 · {routine.subject}</small></span><em>{routine.minutes} 分钟</em></label>; })}{todayTasks.map(task => <label className={`task-row ${task.done ? "done" : ""}`} key={task.id}><input type="checkbox" checked={task.done} onChange={() => setTasks(tasks.map(x => x.id === task.id ? { ...x, done: !x.done } : x))} /><span className="custom-check">✓</span><span className="task-copy"><strong>{task.title}</strong><small>{task.subject}</small></span><em>{task.minutes} 分钟</em></label>)}</> : <Empty text="今天还没有安排任务" />}</div></div>
      <div className="panel timer-panel"><PanelTitle title="专注计时" action="时长统计" onClick={() => setTab("study")} /><div className={`timer-orb ${timer.timerOn && !timer.timerPaused ? "running" : ""} ${timer.timerPaused ? "paused" : ""}`}><small>{timer.timerOn ? (timer.timerPaused ? "本次专注已暂停" : "本次专注") : "今日累计"}</small><strong>{formatClock(timer.timerOn ? timer.activeSeconds : timer.todaySeconds)}</strong><span>{timer.timerOn ? timer.timerTitle : timer.timerTitle || "尚未选择具体任务"}</span></div><select value={timer.timerModule} disabled={timer.timerOn} onChange={e => timer.setTimerModule(e.target.value)}>{Array.from(new Set([...moduleOptions, timer.timerModule])).map(m => <option key={m}>{m}</option>)}</select><button className="primary-button wide" onClick={() => timer.timerOn ? setTab("study") : timer.timerTitle.trim() ? timer.toggleTimer() : setTab("study")}>{timer.timerOn ? "打开计时器" : timer.timerTitle.trim() ? "开始学习" : "选择具体任务"}</button></div></section>
  </div>;
}
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer, moduleOptions }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared; moduleOptions: string[] }) {
  const [date, setDate] = useStoredState<string>("shore-study-selected-date", localISO()), [module, setModule] = useState("资料分析");
  ...
  const selectFocusTask = (choice: ReturnType<typeof taskChoicesFor>[number]) => { if (timer.timerOn) return; timer.setTimerTaskKey(choice.key); timer.setTimerTitle(choice.title); timer.setTimerModule(choice.subject); };
  const selectRecordTask = (choice: ReturnType<typeof taskChoicesFor>[number]) => { setRecordTaskKey(choice.key); setRecordTitle(choice.title); setModule(choice.subject); };
...
function PracticeView({ practices, setPractices, flash, moduleOptions, setModuleOptions, timer }: { practices: Practice[]; setPractices: Dispatch<SetStateAction<Practice[]>>; flash: (x: string) => void; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>>; timer: TimerShared }) {
  ...
  return <div className="page-stack"><section className="page-intro"><div><p className="eyebrow">ACCURACY TRACKER</p><h2>看见每一次进步的轨迹</h2><p>按模块与来源拆分，周变化和月变化统一呈现。</p></div></section>
    <section className="panel option-manager"><div className="option-column"><div><h2>练习来源管理</h2><p>可新增、改名或删除；改名会同步更新历史记录。</p></div><div className="option-list">{sourceOptions.map((x, i) => <div key={`${x}-${i}`}><input defaultValue={x} onBlur={e => renameOption("source", i, e.target.value)} /><button onClick={() => deleteOption("source", i)}>删除</button></div>)}</div><div className="option-add"><input value={newSource} onChange={e => setNewSource(e.target.value)} placeholder="新增来源，如：小程序模考" /><button onClick={() => addOption("source")}>添加</button></div></div><div className="option-column"><div><h2>学习模块管理</h2><p>新增、改名或删除会立即同步到学习计时的模块选择和正确率统计中。</p></div><div className="option-list">{moduleOptions.map((x, i) => <div key={`${x}-${i}`}><input defaultValue={x} onBlur={e => renameOption("module", i, e.target.value)} /><button onClick={() => deleteOption("module", i)}>删除</button></div>)}</div><div className="option-add"><input value={newModule} onChange={e => setNewModule(e.target.value)} placeholder="新增模块，如：政治理论" /><button onClick={() => addOption("module")}>添加</button></div></div></section>

...
function PracticeView({ practices, setPractices, flash, moduleOptions, setModuleOptions, timer, taskPresets, setTaskPresets }: { practices: Practice[]; setPractices: Dispatch<SetStateAction<Practice[]>>; flash: (x: string) => void; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>>; timer: TimerShared; taskPresets: StudyTaskPreset[]; setTaskPresets: Dispatch<SetStateAction<StudyTaskPreset[]>> }) {
  const [range, setRange] = useState<7 | 30>(7), [date, setDate] = useStoredState<string>("shore-practice-selected-date", localISO()), [source, setSource] = useState("超大杯"), [module, setModule] = useState("资料分析"), [correct, setCorrect] = useState(18), [total, setTotal] = useState(20), [minutes, setMinutes] = useState(24);
  ...
  const renameOption = (kind: "source" | "module", index: number, value: string) => { const clean = value.trim(); if (!clean) return; const list = kind === "source" ? sourceOptions : moduleOptions, old = list[index]; if (list.some((x, i) => i !== index && x === clean)) return; if (kind === "source") { setSourceOptions(list.map((x, i) => i === index ? clean : x)); setPractices(practices.map(x => x.source === old ? { ...x, source: clean } : x)); if (source === old) setSource(clean); } else { setModuleOptions(list.map((x, i) => i === index ? clean : x)); setPractices(practices.map(x => x.module === old ? { ...x, module: clean } : x)); setTaskPresets(taskPresets.map(item => item.subject === old ? { ...item, subject: clean } : item)); if (module === old) setModule(clean); if (!timer.timerOn && timer.timerModule === old) timer.setTimerModule(clean); } };
  const deleteOption = (kind: "source" | "module", index: number) => { const list = kind === "source" ? sourceOptions : moduleOptions; if (list.length <= 1) return; const old = list[index], next = list.filter((_, i) => i !== index); if (kind === "source") { setSourceOptions(next); if (source === old) setSource(next[0]); } else { setModuleOptions(next); setTaskPresets(taskPresets.map(item => item.subject === old ? { ...item, subject: next[0] } : item)); if (module === old) setModule(next[0]); if (!timer.timerOn && timer.timerModule === old) timer.setTimerModule(next[0]); } flash(`已删除选项“${old}”，历史记录仍保留`); };

...
function Plan({ tasks, setTasks, routines, setRoutines, flash }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; flash: (x: string) => void }) {
  ...
  const [title, setTitle] = useState(""), [subject, setSubject] = useState("资料分析"), [minutes, setMinutes] = useState(45);
...
function StudyTime({ sessions, setSessions, tasks, routines, flash, timer, moduleOptions, setModuleOptions }: { sessions: StudySession[]; setSessions: Dispatch<SetStateAction<StudySession[]>>; tasks: Task[]; routines: DailyRoutine[]; flash: (x: string) => void; timer: TimerShared; moduleOptions: string[]; setModuleOptions: Dispatch<SetStateAction<string[]>> }) {
  ...
  const taskChoicesFor = (targetDate: string) => [
    ...routines.filter(r => r.month === targetDate.slice(0, 7) && routineApplies(r, targetDate)).map(r => ({ key: makeTaskKey("routine", r.id), title: r.title, subject: r.subject, start: r.plannedStart, end: r.plannedEnd, timeLabel: formatPlannedSlots(r), kind: "每日任务" })),
    ...tasks.filter(t => normalizedDate(t.date) === targetDate).map(t => ({ key: makeTaskKey("task", t.id), title: t.title, subject: t.subject, start: t.plannedStart, end: t.plannedEnd, timeLabel: formatPlannedSlots(t), kind: "单日任务" })),
  ];
  ...
  const selectFocusTask = (choice: ReturnType<typeof taskChoicesFor>[number]) => { if (timer.timerOn) return; timer.setTimerTaskKey(choice.key); timer.setTimerTitle(choice.title); timer.setTimerModule(ensureSharedModule(choice.subject)); };

...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; timer: TimerShared }) {
  ...
  const plannedBlocks = planned.flatMap(item => item.slots.map((slot, slotIndex) => ({ ...item, start: slot.start, end: slot.end, slotIndex })));
  ...
  const timelineDuration = (start?: string, end?: string) => {
    const startMinute = Math.max(360, Math.min(1440, timeMinutes(start) ?? 360));
    const endMinute = Math.max(startMinute + 1, Math.min(1440, timeMinutes(end) ?? startMinute + 1));
    return endMinute - startMinute;
  };
  const timelineHeight = (start?: string, end?: string) => (timelineDuration(start, end) / 1080) * 100;

...
type Idiom = { id: number; word: string; meaning: string; example: string; level: "未掌握" | "模糊" | "已掌握"; source?: string; meaningHighlights?: string[]; ocrNote?: string };
...
function DailyPlan({ tasks, setTasks, routines, setRoutines, sessions, timer }: { tasks: Task[]; setTasks: Dispatch<SetStateAction<Task[]>>; routines: DailyRoutine[]; setRoutines: Dispatch<SetStateAction<DailyRoutine[]>>; sessions: StudySession[]; timer: TimerShared }) {
  ...
  return <div className="page-stack"><section className="page-intro daily-intro"><div><p className="eyebrow">DAILY TIMELINE</p><h2>计划时间与真实投入并排看</h2><p>左边是预计安排，右边由学习计时自动生成实际完成时段。</p></div><input type="date" value={date} onChange={e => setDate(e.target.value)} /></section>
    ...
    <section className="panel schedule-dock"><div className="schedule-dock-head"><div><h2>待安排与时间设置</h2><p>未设时间的任务在这里直接安排，不必穿过时间轴来回翻页。</p></div><div className="timeline-zoom"><span>时间轴缩放 {timelineZoom}%</span><input type="range" min="65" max="160" step="5" value={timelineZoom} onChange={e => setTimelineZoom(Number(e.target.value))} /><button onClick={() => setTimelineZoom(100)}>恢复标准</button></div></div>
    ...
    <section className="panel daily-timeline-panel"><div className="timeline-head"><span>预计任务安排</span><b>时间</b><span>实际学习记录</span></div><div className="daily-timeline" style={{ height: `${Math.round(1080 * timelineZoom / 100)}px` }}><div className="timeline-side planned-side">{planned.filter(x => x.start).map(item => <article key={`${item.kind}${item.id}`} className={item.done ? "done" : ""} style={{ top: `${timelineTop(item.start)}%`, minHeight: `${timelineHeight(item.start, item.end)}%` }}><button className="timeline-check" onClick={() => togglePlanned(item)}>{item.done ? "✓" : ""}</button><div onClick={() => beginEdit(item)}><strong>{item.title}</strong><small>{item.start}–{item.end} · {item.subject}</small></div></article>)}</div><div className="timeline-axis">{Array.from({ length: 19 }, (_, i) => i + 6).map(h => <div key={h}><span>{pad(h)}:00</span><i /></div>)}</div><div className="timeline-side actual-side">{actual.filter(x => x.startTime).map(item => <article key={item.id} className={item.running ? "running" : ""} style={{ top: `${timelineTop(item.startTime)}%`, minHeight: `${timelineHeight(item.startTime, item.endTime)}%` }}><div><strong>{item.module}</strong><small>{item.startTime}–{item.endTime || "进行中"} · {formatHours(item.seconds)}</small></div></article>)}</div></div></section>
  </div>;
}
